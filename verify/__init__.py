...
class Verify( commands.Cog ):
    ...
    async def createGlobalVerifyMessage( self ) -> None:
        verifyChannel: discord.TextChannel = self.bot.get_guild(1220164296999436379).get_channel(1223587591677415514)
    ...
    async def verifyUser(self, member: discord.Member) -> None:
        ...
        verifyChannel: (GuildChannel | PrivateChannel | discord.Thread | None) = self.bot.get_channel(1220164296999436379)
        ...
        if self.sendOTP(userEmail, currentOTPCode):
            ...
            if verifyComplete == False:
                ...
                self.bot.sql.execute(f'insert or ignore into User(discord_id, email) values({member.id}, \"{userEmail}\")')
                # welcome channel
                welcomeChannel: discord.TextChannel = self.bot.get_guild(1220164296999436379).get_channel(1220164297506816080)
                # when verified complete
                serverGuild: discord.Guild = self.bot.get_guild(1220164296999436379)
